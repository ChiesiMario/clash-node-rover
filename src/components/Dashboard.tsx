        ...
        <div className="p-6 rounded-xl border border-border bg-muted/30 space-y-4 transition-colors hover:bg-muted/50">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-sm text-muted-foreground">
              {!apiUrl 
                ? t('dashboard.api_not_set', 'No API Address Set')
                : status.api_connected 
                  ? t('dashboard.api_connected', 'API Connected') 
                  : t('dashboard.api_disconnected', 'API Disconnected')}
            </h3>
            {status.api_connected ? (
          ...
          <div className="text-2xl font-semibold truncate" title={apiUrl}>
